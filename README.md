...
| **TradingBrain** | Pull-based Aggregator über die 15 Agenten: jeder contributes ein typed `AgentSignal` (score / direction / confidence / evidence), Brain produziert eine **bullish/bearish/neutral/halt** Entscheidung. **Veto-Layer** — `risk_security_agent` (DANGER) und `qa_test_agent` (kritische Compliance-Issues) können Trading sofort haltieren. CLI: `tao-swarm trade brain`. Doku: [`docs/trading_brain.md`](docs/trading_brain.md). |
