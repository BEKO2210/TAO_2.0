...
| Bereich | Stand |
|---|---|
| **Agenten** | 15 deterministische Python-Agenten als koordiniertes Experten-Team (kein LLM intern). **Jeder Agent hat eine echte Datenquelle** — entweder ein Live-Collector (chain/wallet/market) oder ein Upstream-Pull über den AgentContext-Bus. Lineage als Vertrag in `tests/test_agent_data_lineage.py` durchgesetzt. Plus eigene **Plug-ins** aus deinem Repo. Doku: [`docs/agent_lineage.md`](docs/agent_lineage.md). |
| **TradingCouncil** | Pull-based Aggregator über die 15 Agenten: jeder contributes ein typed `AgentSignal` (score / direction / confidence / evidence), Council produziert eine **bullish/bearish/neutral/halt** Entscheidung. **Veto-Layer** — `risk_security_agent` (DANGER) und `qa_test_agent` (kritische Compliance-Issues) können Trading sofort haltieren. CLI: `tao-swarm trade council`. Doku: [`docs/trading_council.md`](docs/trading_council.md). |
